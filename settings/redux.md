...
// 그냥 함수
export const 함수이름1 = (파라미터: 타입) => (dispatch: Dispatch<CustomActionDispatchTypes>) => {
...
// 비동기 함수 예시
export const 함수이름2 = (파라미터: 타입) => async (dispatch: Dispatch<CustomActionDispatchTypes>) => {
  try {
...
`/index.tsx`에 store를 import하고 Provider에 세팅한다.
...
이제 사용할 곳에 store와 action을 import 하고 사용하면 된다.

```tsx
import { RootStore } from './store';
import { 함수이름1 } from "./actions/CustomActinos"

function App() {
  const dispatch = useDispatch();

  // action 사용하기
  const 이_파일에서_사용되는_메소드_이름 = () => {
    dispatch(함수이름1(파라미터))
  }

  // store 가져오기
  const state = useSelector((state: RootStore) => state.루트리듀서_객체의_키값)
}
